...
class Hc(Processor):
    ...
    def calculate_invariance(self, z):
        z_mean = z.mean(dim=2)  # Shape: (classes, nb_concepts)
        
        concept_invariance = torch.zeros(self.sae_manager.nb_concepts).to(z.device)

        for i in range(self.sae_manager.nb_concepts):
            if z_mean[:, i].sum() == 0:
                continue

            # Calculate activation distribution across CLASSES
            score = z_mean[:, i] / z_mean[:, i].sum()
            
            # Entropy normalized by the number of classes
            entropy = -1 / torch.log(torch.tensor(float(self.classes))) * (score * torch.log(score + 1e-12)).sum()
            concept_invariance[i] = entropy

        
        invariance = concept_invariance.unsqueeze(0).repeat(self.classes, 1)
        return invariance
    ...
    def process(self):
        ...
        self.dump(invariance, "Hc")
        self.dump(activations, "Hc_mean_acts")
        self.dump(counts, "Hc_counts")
